...
export default function SipCalculatorPage() {
  ...
  const seoContent = getSEOContent('sip-calculator');

  // Core calculations
  const { totalInvested, estimatedReturns, totalValue, yearlySchedule } = useMemo(() => {
    const P = Math.max(0, monthlyInvestment);
    const rAnnual = Math.max(0, expectedReturn);
    const nYears = Math.max(1, durationYears);
    ...
    for (let year = 1; year <= nYears; year++) {
      ...
      schedule.push({
        ...
        returns: Math.max(0, currentFV - currentInvested),
        value: currentFV,
      });
    }
  ...
  // Chart data
  const pieData = [
    { name: 'Invested Amount', value: totalInvested, color: '#818cf8' }, // indigo-400
    { name: 'Est. Returns', value: estimatedReturns, color: '#312e81' }, // indigo-900
  ];
        ...
        <div className="utilo-container">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.3fr_1fr] lg:items-start">
            ...
            <div className="space-y-8">
              <Card className="border-0 bg-white shadow-xl shadow-slate-200/50 ring-1 ring-slate-100">
                ...
                <CardContent className="space-y-8 pt-4">
                  ...
                  <SliderField
                    ...
                    helper="Amount you plan to invest every month."
                    value={monthlyInvestment}
                    min={500}
                    max={100000}
                    step={500}
                    displayValue={formatMoney(monthlyInvestment)}
                  ...
                  <SliderField
                    ...
                    helper="Annual expected return from your mutual fund."
                  ...
                  <SliderField
                    ...
                    helper="Number of years you plan to keep investing."
                    value={durationYears}
                    min={1}
                    max={40}
                    step={1}
              ...
              <Card className="border-0 bg-white shadow-xl shadow-slate-200/50 ring-1 ring-slate-100">
                <CardHeader>
                  <CardTitle className="text-xl">Wealth Growth Simulator</CardTitle>
                  <CardDescription>Visualizing your invested amount versus total accumulated value.</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="h-64 w-full mt-4">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={yearlySchedule} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        ...
                        <RechartsTooltip 
                          formatter={(value: number, name: string) => [formatMoney(value), name === 'value' ? 'Total Value' : 'Invested']}
            ...
            <div className="lg:sticky lg:top-24 space-y-6">
              <Card className="overflow-hidden border-0 bg-indigo-950 text-white shadow-2xl shadow-indigo-900/20">
                <CardHeader className="border-b border-indigo-900/50 bg-indigo-950/50 pb-6 pt-8">
                  <p className="text-sm font-semibold tracking-widest text-indigo-300 uppercase mb-2">Total Value</p>
                  <div className="text-5xl font-bold tracking-tight">
                    {formatMoney(totalValue)}
                  </div>
                  <p className="mt-2 text-sm text-indigo-200">Estimated wealth after {durationYears} years</p>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="grid grid-cols-2 divide-x divide-indigo-800/50 bg-indigo-900/20">
                    <div className="p-6">
                      <p className="text-xs font-semibold tracking-widest text-indigo-300 uppercase mb-2">Invested</p>
                      <p className="text-xl font-semibold text-white">{formatMoney(totalInvested)}</p>
                    </div>
                    <div className="p-6">
                      <p className="text-xs font-semibold tracking-widest text-indigo-300 uppercase mb-2">Est. Returns</p>
                ...
                <table className="w-full text-left text-sm text-slate-600">
                  <thead className="bg-slate-50 text-slate-900">
                    <tr>
                      ...
                      <th className="px-6 py-4 font-semibold text-right">Invested Amount</th>
                      <th className="px-6 py-4 font-semibold text-right">Returns Earned</th>
          ...
          {seoContent && (
            <div className="print-hide">
              <SEOContentSection seoContent={seoContent} toolName="SIP Calculator" slug="sip-calculator" />
            </div>
          )}
        </div>
      </main>
